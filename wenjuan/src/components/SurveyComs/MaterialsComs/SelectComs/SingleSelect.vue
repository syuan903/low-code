<template>
  <div :class="{ 'text-center': computedState.position }">
    ...
    <div class="radio-group">
      <el-radio-group v-model="radioValue" @click.stop @change="updateAnswer">
        <el-radio v-for="(item, index) in computedState.options" :key="index" :value="item">{{
          item
        }}</el-radio>
      </el-radio-group>
    </div>
...
const updateAnswer = () => {
  ...
  // 通过自定义事件的形式
  // 传递的内容是当前选中的值
  emit('updateAnswer', radioValue.value)
}
</script>
